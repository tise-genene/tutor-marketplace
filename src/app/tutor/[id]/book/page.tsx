...
export default function BookTutorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const selectedSubjectData = tutor.subjects.find(s => s.id === selectedSubject);
      ...
      if (result.success) {
        ...
        alert(result.error?.message || 'Booking failed');
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h1 className="text-2xl font-bold mb-4">Book Session with {tutor.name}</h1>
